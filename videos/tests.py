...
class ConvertToHLSTaskTest(TestCase):
    ...
    @patch('videos.tasks.subprocess.run', side_effect=Exception('ffmpeg not found'))
    @patch('videos.tasks.os.makedirs')
...
class VideoPostDeleteSignalTest(TestCase):
    ...
    def setUp(self):
        ...
        with patch('videos.signals.django_rq.get_queue', return_value=MagicMock()):
    ...
    @patch('videos.signals.django_rq.get_queue')
    @patch('videos.signals.shutil.rmtree')
    @patch('videos.signals.os.path.isdir', return_value=True)
    @patch('videos.signals.os.remove')
    @patch('videos.signals.os.path.isfile', return_value=True)
    ...
    @patch('videos.signals.django_rq.get_queue')
    @patch('videos.signals.shutil.rmtree')
    @patch('videos.signals.os.path.isdir', return_value=False)
    @patch('videos.signals.os.remove')
    @patch('videos.signals.os.path.isfile', return_value=False)
